...
class AttrEncryptedTest < Minitest::Test
  ...
  def test_should_store_email_in_legacy_encrypted_attributes
    assert User.legacy_encrypted_attributes.include?(:email)
  end

  def test_should_not_store_salt_in_legacy_encrypted_attributes
  ...
  def test_should_evaluate_a_key_passed_as_a_symbol
    ...
    encrypted =  Encryptor.legacy_encrypt(:value => 'testing', :key => SECRET_KEY, :iv => @user.ssn_encrypted_iv.unpack("m").first, :salt => @user.ssn_encrypted_salt.unpack("m").first )
  ...
  def test_should_evaluate_a_key_passed_as_a_proc
    ...
    encrypted = Encryptor.legacy_encrypt(:value => 'testing', :key => SECRET_KEY, :iv => @user.crypted_password_test_iv.unpack("m").first, :salt =>  @user.crypted_password_test_salt.unpack("m").first)
  ...
  def test_should_inherit_legacy_encrypted_attributes
  ...
  def test_should_encrypt_with_true_if
    ...
    encrypted = Encryptor.legacy_encrypt(:value => 'testing', :key => SECRET_KEY, :iv => @user.encrypted_with_true_if_iv.unpack("m").first, :salt => @user.encrypted_with_true_if_salt.unpack("m").first)
  ...
  def test_should_encrypt_with_false_unless
    ...
    encrypted = Encryptor.legacy_encrypt(:value => 'testing', :key => SECRET_KEY, :iv => @user.encrypted_with_false_unless_iv.unpack("m").first, :salt => @user.encrypted_with_false_unless_salt.unpack("m").first)
  ...
  def test_legacy_encrypted_attributes_state_is_not_shared
    user = User.new
    user.ssn = '123456789'
