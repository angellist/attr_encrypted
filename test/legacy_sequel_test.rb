...
class LegacyHuman < Sequel::Model(:legacy_humans)
  ...
  attr_encrypted :credentials, :key => Proc.new { |human| Encryptor.legacy_encrypt(:value => human.salt, :key => 'some private key', insecure_mode: true, algorithm: 'aes-256-cbc') }, :marshal => true, mode: :single_iv_and_salt
